...
import {encodeAminoSignRpcRequestParams, encodeDirectSignRpcRequestParams} from "./encode";
 ...
 */
export async function rpcCosmosGetAccounts(client: SignClient, session: SessionTypes.Struct, chainId: string): Promise<AccountData[]> {
...
export async function rpcCosmosSignDirect(
  ...
  chainId: string, signerAddress: string, signDoc: SignDoc
...
export async function rpcCosmosSignAmino(
  client: SignClient, session: SessionTypes.Struct,
  chainId: string, signerAddress: string, signDoc: StdSignDoc
): Promise<SignAminoRpcResponseParams> {
  return await client.request<SignAminoRpcResponseParams>({
    topic: session.topic,
    chainId,
    request: {
