...
export function setupSupplyExtension(base: QueryClient): SupplyExtension {
  ...
  return {
    supply: {
      circulatingSupply: async (denom: string, dividerExponent?: Long) => queryService.Circulating({
          denom,
          dividerExponent: dividerExponent || Long.ZERO,
        }),
      totalSupply: async (denom: string, dividerExponent?: Long) => queryService.Total({
          denom,
          dividerExponent: dividerExponent || Long.ZERO,
        }),
    },
